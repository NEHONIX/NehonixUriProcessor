...
class NDS {
  ...
  static decodeAnyToPlaintext(
    ...
    while (iterations < maxIterations && result !== lastResult) {
      ...
      // If plaintext with high confidence, stop
        ...
        try {
          ...
          switch (detection.mostLikely) {
            case "base64":
              ...
              base64Input = base64Input.replace(/-/g, "+").replace(/_/g, "/");
              decoded = NehonixSharedUtils.decodeB64(base64Input);
              break;
            case "hex":
            case "rawHexadecimal":
              if (/^[0-9A-Fa-f]+$/.test(result) && result.length % 2 === 0) {
                decoded = NDS.decodeRawHex(result);
              } else {
                decoded = result;
              }
              break;
            case "asciihex":
              decoded = NDS.decodeAsciiHex(result);
              break;
            default:
              decoded = NDS.decode({
                input: result,
          ...
          if (
            ...
            break; // Stop if decoding doesn't produce a valid result
          }
        } catch (e) {
          console.warn(`Error decoding ${result}: ${e}`);
          break;
        }
      } else {
          ...
          try {
            switch (detection.mostLikely) {
              case "base64":
                ...
                base64Input = base64Input.replace(/-/g, "+").replace(/_/g, "/");
                decodedValue = NehonixSharedUtils.decodeB64(base64Input);
                // Handle Base64 values containing '&' (e.g., 'true&')
                if (decodedValue.includes("&")) {
                  decodedValue = decodedValue.split("&")[0];
                }
                // Recursively decode if needed
                if (
              ...
              case "rawHexadecimal":
                ...
                break;
              case "asciihex":
                NDS.decodeAsciiHex(value);

              case "hex":
                decodedValue = NDS.decodeHex(value);
                break;
              case "unicode":
                decodedValue = NDS.decodeUnicode(value);
                break;
              case "htmlEntity":
                decodedValue = NDS.decodeHTMLEntities(value);
                break;
              case "punycode":
                decodedValue = NDS.decodePunycode(value);
                break;
              case "jwt":
                decodedValue = NDS.decodeJWT(value);
                break;
              case "urlSafeBase64":
                return NDS.decodeUrlSafeBase64(value);
              case "base32":
                return NDS.decodeBase32(value);
              case "decimalHtmlEntity":
                return NDS.decodeDecimalHtmlEntity(value);
              case "rot13":
                return NDS.decodeRot13(value);
              case "asciioct":
                return NDS.decodeAsciiOct(value);
              case "jsEscape":
                return NDS.decodeJsEscape(value);
              case "cssEscape":
                return NDS.decodeCssEscape(value);
              case "utf7":
                return NDS.decodeUtf7(value);
              case "quotedPrintable":
                return NDS.decodeQuotedPrintable(value);
              default:
                decodedValue = NDS.decode({
                  input: value,
                  encodingType: detection.mostLikely as ENC_TYPE,
            ...
            if (printableRatio < 0.7) {
              decodedValue = value; // Revert if decoding produces garbage
            } else {
              modified = true;
            }
          } catch (e) {
            console.warn(`Failed to decode parameter ${key}=${value}: ${e}`);
            decodedValue = value;
          }
        } else if (value.includes("%")) {
